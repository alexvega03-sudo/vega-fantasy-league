...
export function WeeklyBreakdown() {
  const { getWeeklyBreakdown, contestants } = useGame();
  ...
  // Generate week options (1 through CURRENT_WEEK)
  const weekOptions = Array.from({ length: CURRENT_WEEK }, (_, i) => i + 1);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200 bg-gray-50">
          <h3 className="text-lg font-semibold text-gray-900">All Contestants - Week {selectedWeek}</h3>
        </div>
                  ...
                  <tr key={contestant.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`
                        ...
                        ${contestant.tribe === 'Blue' ? 'bg-blue-100 text-blue-800' : 'bg-red-100 text-red-800'}
                      `}>
                        {contestant.tribe}
                      </span>
                    </td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`
                        inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                        ${contestant.isEliminated ? 'bg-gray-100 text-gray-800' : 'bg-green-100 text-green-800'}
                      `}>
